...
if (isset($_POST['add-officer'])) {
  ...
  #validations
  if (empty($title)) {
    $_SESSION['add-officer'] = "Please title cannot be null";
  } elseif (empty($name)) {
    $_SESSION['add-officer'] = "Please provide name";
  } elseif (empty($marital_status)) {
    $_SESSION['add-officer'] = "Please select marital status";
  } elseif (empty($residence)) {
    $_SESSION['add-officer'] = "Please enter residence";
  } elseif (empty($group_name)) {
    $_SESSION['add-officer'] = "Please select group";
  } elseif (empty($contact)) {
    $_SESSION['add-officer'] = "Please enter phone number";
  } elseif (empty($profile['name'])) {
    $_SESSION['add-officer'] = "Please select profile";
  } else {
    #verifying if officer already exist
    $verify_query = " SELECT * FROM officers WHERE name='$name' OR email='$email' OR contact='$contact' ";
    $verify_results = mysqli_query($connection, $verify_query);
    if (mysqli_num_rows($verify_results) > 0) {
      $_SESSION['add-officer'] = "Please name, email or phone already exist";
    } else {
      #working on image
      $time = time();
      ...
      if (in_array($extension, $allowed_files)) {
        ...
        if ($profile['size'] > 1000000) {
          $_SESSION['add-officer'] = "Please file size is big. 4mb Max";
        } else {
          move_uploaded_file($profile_tmp_name, $profile_destination_path);
        }
      } else {
        $_SESSION['add-officer'] = "File is not supported";
      }
    }
  }

  #checking if there was an error
  if (isset($_SESSION['add-officer'])) {
    $_SESSION['add-officer-data'] = $_POST;
    ...
    $insert_query = " INSERT INTO officers(title, name, marital_status, residence, group_name, email, contact, ministry, status, profile, date) 
    ...
    if (mysqli_errno($connection)) {
      ...
      $_SESSION['add-officer-success'] = "New officers added successfully";
      header('location:' . ROOT_URL . 'officers.php');
    }
  }
} else {
