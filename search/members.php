...
if (isset($_GET['search-btn'])) {
  ...
  if (empty($_GET['search'])) {
    ...
    $query = " SELECT * FROM members WHERE CONCAT(title, name, group_name, residence,status,marital_status) LIKE '%$search%' ORDER BY id DESC";
  ...
  <header class="header">
    ...
    <div class="search__container">
      <form action="" method="get" autocomplete="off" class="form">
        ...
        <a href="<?= ROOT_URL ?>members.php" class="search-btn">Members</a>
      </form>
    </div>
              ...
              <tr>
                ...
                <td style="text-align: center;">
                  <?php if ($member['status'] === 'Active') : ?>
                    <a href="<?= ROOT_URL ?>page/member-profile.php?member_profile_id_numder=<?= $member['id'] ?>" target="_parent" title="Active" class="status-btn active"><i class="fa fa-check-circle"></i></a>
                  <?php elseif ($member['status'] === 'Normal') : ?>
                    <a href="<?= ROOT_URL ?>page/member-profile.php?member_profile_id_numder=<?= $member['id'] ?>" target="_parent" title="Normal" class="status-btn normal"><i class="fa fa-exclamation-circle"></i></a>
                  <?php else : ?>
                    <a href="<?= ROOT_URL ?>page/member-profile.php?member_profile_id_numder=<?= $member['id'] ?>" target="_parent" title="InActive" class="status-btn inactive"><i class="fa fa-times-circle"></i></a>
                  <?php endif ?>
                </td>
                <td>
                  <span class="td-action">
                    <a target="_parent" title="Edit" href="<?= ROOT_URL ?>edit/edit-member.php?edit_member_id_number=<?= $member['id'] ?>" class="atn-btn edit"><span><i class="fa fa-user-edit"></i></span></a>
                    <a target="_parent" title="Delete" href="<?= ROOT_URL ?>logic/delete-member.php?delete_member_id_number=<?= $member['id'] ?>" class="atn-btn delete"><span><i class="fa fa-trash-can"></i></span></a>
                  </span>
                </td>
              </tr>
